...
def main():
    ...
    elif 'ade20k' in args.data_type:
        val_data = dataset.Ade20kSegmentation(root=args.data_root, split='val', transform=composed_transforms_ts)
    elif args.data_type == 'cityscape_full':
        val_data = dataset.CityScapesSegmentation(root=args.data_root, split='val', label_type='full', transform=composed_transforms_ts)
    elif args.data_type == 'cityscape_select':
        val_data = dataset.CityScapesSegmentation(root=args.data_root, split='val', label_type='select', transform=composed_transforms_ts)
    ...
    elif args.data_type == 'grabcut':
        val_data = dataset.GrabCutSegmentation(root=args.data_root, transform=composed_transforms_ts)
    else:
    ...
    with torch.no_grad():
        ...
        for ii, sample_batched in enumerate(val_loader):
            ...
            for jj in range(preds.size()[0]):
                ...
                # Evaluate
                if use_void_pixels:
                    j_s, j_nu, j_de = jaccard(gt, pred, void_pixel)
    ...
    logger.info(eval_result['all_fscore'].avg)
    ...
    logger.info("per_categ_IoU: ")
    logger.info(sort_dict(eval_result["per_categ_jaccard"]))
    pcj_list = list(eval_result["per_categ_jaccard"].values())
    logger.info(sum(pcj_list) / len(pcj_list))
    logger.info("per_categ_fscore: ")
    logger.info(sort_dict(eval_result["per_categ_fscore"]))
    pcf_list = list(eval_result["per_categ_fscore"].values())
    logger.info(sum(pcf_list) / len(pcf_list))
    logger.info("per_categ_precision: ")
    logger.info(sort_dict(eval_result["per_categ_precision"]))
    pcf_list = list(eval_result["per_categ_precision"].values())
    logger.info(sum(pcf_list) / len(pcf_list))
    logger.info("per_categ_recall: ")
    logger.info(sort_dict(eval_result["per_categ_recall"]))
    pcf_list = list(eval_result["per_categ_recall"].values())
    logger.info(sum(pcf_list) / len(pcf_list))
